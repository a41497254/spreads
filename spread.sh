...
print(f"{'Source':<22} {'Price':<14} Implied Val")
print(f"{'-'*22} {'-'*14} {'-'*11}")
print(f"{'VNTL mark (HL perp)':<22} {f'{vmark} B':<14} {fmt(vval)}")
print(f"{'VNTL oracle':<22} {f'{vorac} B':<14} -")
print(f"{'PreStocks token':<22} {f'${pp:.2f}/tok':<14} {fmt(pval)}")
print(f"{'BingX VNTL perp':<22} {f'{bvntl} USDT' if bvntl else 'n/a':<14} -")
print(f"{'BingX PreStocks perp':<22} {f'{bpre} USDT' if bpre else 'PAUSED':<14} -")
print(f"{'OKX SPACEX-USDT-SWAP':<22} {f'{okx} USDT' if okx else 'n/a':<14} -")
